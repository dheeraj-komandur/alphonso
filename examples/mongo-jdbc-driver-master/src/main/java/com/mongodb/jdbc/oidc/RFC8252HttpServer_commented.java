...
public class RFC8252HttpServer {
    ...
    // (AI Comment) - Constructor initializes the template engine and the OIDC response queue.
    ...
    /**
     // (AI Comment) - Retrieves an OIDC response from the queue, waiting up to 300 seconds.
    ...
    // (AI Comment) - Stops the HTTP server if it is currently running.
    public void stop() {
        if (server != null) {
            server.stop(0);
        }
    ...
    // (AI Comment) - Creates and configures the template engine for rendering HTML responses.
    /**
    ...
    // (AI Comment) - Handles HTTP requests for the callback and redirect endpoints.
    ...
    // (AI Comment) - Handles HTTP requests for the accepted endpoint.
